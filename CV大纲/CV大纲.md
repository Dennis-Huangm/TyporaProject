...
<img src="./assets\image-20250626171241548.png" alt="image-20250626171241548" style="zoom:80%;" />
...
<img src="./assets/image-20250626171312520-1751209503636-43.png" alt="image-20250626171312520"  />
...
![image-20250626234201918](./assets/image-20250626234201918.png)
...
\end{aligned}
$$
<img src="./assets/image-20250627163525024.png" alt="image-20250627163525024" style="zoom: 80%;" />
...
![image-20250627140340273](./assets/image-20250627140340273.png)
...
$$
![image-20250627144131621](./assets/image-20250627144131621.png)
...
<img src="./assets/image-20250627150645630.png" alt="image-20250627150645630" style="zoom: 80%;" />
...
![image-20250627150900728](./assets/image-20250627150900728.png)
...
![image-20250627153137435](./assets/image-20250627153137435.png)
...
![image-20250627161623180](./assets/image-20250627161623180.png)

- **使⽤双阈值检测与边缘连接**：使⽤双阈值（高阈值和低阈值）将像素点分为三类：**强边缘、弱边缘和非边缘**。高于**高阈值的像素点被分类为强边缘，低于低阈值的被分类为非边缘，介于两者之间的被分类为弱边缘**。然后，如果**弱边缘像素的某个邻域像素是强边缘像素，则该弱边缘像素被保留为真实的边缘**。

  <img src="./assets/image-20250627161746564.png" alt="image-20250627161746564" style="zoom:80%;" />
...
- 重复$N$次：
  ...
  - 拟合模型：使⽤这$s$个点来拟合⼀条直线。

  - 找内点：在剩余的点中，找到与这条直线的距离小于$t$的点，这些点被称为内点($t$是距离阈值)。
  - 验证内点数量： 如果内点的数量大于或等于$d$，则接受这条直线，并使⽤所有内点重新拟合直线($d$是数量阈值)。
...
优点：**简单、能有效解决多种问题、计算效率更高、减少参数调整**；缺点：**对初始参数敏感、需要调整的参数较多、当问题很复杂，内点率很低时，算法需要的迭代次数很多，甚⾄⽆法结束**。
...
- **遍历每个边缘点 $(x, y)$：**
  ...
  - 对于每一个$\theta_i$，利用公式$p=x\cos\theta+y\sin\theta$计算对应的$\rho_j$。
...
- **在每个像素点周围的高斯窗口内计算二阶矩矩阵**：使⽤高斯窗口对**每个像素点的二阶矩矩阵**进行**加权求和**，得到矩阵$M$。
...
Blob检测的目标：**使⽤不同尺度的拉普拉斯核卷积图像，获取响应幅值，当幅值出现极值时认为获得了较好的尺度特征。**
...
经尺度归一化后的结果为：
$$
\nabla_{\mathsf{norm}}^2g=\sigma^2\left(\frac{\partial^2g}{\partial x^2}+\frac{\partial^2g}{\partial y^2}\right)
$$
...
<img src="./assets/image-20250629115121093.png" alt="image-20250629115121093" style="zoom: 45%;" />

我们可以令：
$$
\nabla_{\mathsf{norm}}^2g=0
$$
解得：$\sigma=r/\sqrt{2}。$即圆的半径$r=\sqrt{2}\sigma$。可以看出，**较大的$\sigma$对应的拉普拉斯核可以检测出较大的物体/信号**，反之，可以检测出较小的物体/信号。
...
<img src="./assets\image-20250629204919196.png" alt="image-20250629204919196" style="zoom:50%;" />
...
我们可以通过在图中找到**最小割**来进行分割，期望**剔掉的边的相似性之和是最小的**。归一化图割每次**只分两类**（要么$A$类要么$B$类，二分类模型）。
...
**这个问题可以通过利用所有与该段关联边的权重对割进行归一化来解决**。**归一化割**代价的计算公式为：
$$
...
- 我们最终优化的目标是找到令**归一化割代价**值**最小**的$y$（找到一条切割边满足上面的表达式）：
...
(D-W)y=\lambda Dy
$$
最终的结果==**第二小的特征值对应的特征向量**==（第一小是0），且该特征向量并非只包含$0,1$，因此我们可以设置一个阈值，只有当向量的元素大于这个阈值时才归为第一类，反之则为第二类。同时，我们还可以将经阈值处理后的向量带回到公式中，看它是否为最小值，以此判断阈值的选择是否合理。
